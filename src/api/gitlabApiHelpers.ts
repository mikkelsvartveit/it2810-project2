...
export const gitlabApiURIFromProjectLink = (projectLink: string) => {
    // bad projectLink, return null
    if (!projectLink.match(/https:\/\/.*gitlab\..*?\/.+/g)) return null;

    const regex = /(https:\/\/.*gitlab\..*?)\/(.*)/g;
    const match = projectLink.matchAll(regex);
    let [_, URI, id] = match.next().value as RegExpMatchArray;
...
export const getCommits = async (
    ...
    const uri = `${projectURI}/repository/commits`;
    const fetched = await fetchRequestWithToken(uri, accessToken);
